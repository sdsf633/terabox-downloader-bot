...
Join @tbx_server For Updates
[Source Code](https://github.com/r0ld3x/terabox-downloader-bot) """
    check_if = await is_user_on_chat(bot, "@tbx_server", m.peer_id)
    if not check_if:
        return await m.reply("Please join @tbx_server then send me the link again.")
    check_if = await is_user_on_chat(bot, "@tbx_chats", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @tbx_chats then send me the link again."
        )
